...
local function replace(kind, default, fn)
    ...
    core.command_view:enter("find to replace " .. kind, function(old)
        ...
        if n == 1 then
            ...
            core.log("replaced one instance of %s %q with %q", n, kind, old, new)
        else

            core.log("replaced %d instances of %s %q with %q", n, kind, old, new)
        end
    end)
  end)
end
...
command.add(has_selection, {
    ...
    ["find-replace:select-previous"] = function()

        local l1, c1, l2, c2 = doc():get_selection(true)
        local text = doc():get_text(l1, c1, l2, c2)
        local l1, c1, l2, c2 = search.find(doc(), l2, c2, text, {wrap = true})
        if l1 then doc():set_selection(l2, c2, l1, c1) end
    end
})
