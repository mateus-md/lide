...
function treeview:on_mouse_pressed(button, x, y)
    ...
    if not self.hovered_item then return
    elseif self.hovered_item.type == "dir" then
        ...
        core.try(function()
            ...
            core.open_doc(self.hovered_item.filename))
        end)
    end
end
...
function treeview:update()
    ...
    local iwdt = treeview:get_item_height()

    -- Lock scrolling when treeview is short --
    if core.project_files then

        local all = {}
        local cnt
        for f in self:each_item() do all[#all + 1] = f end

        cnt = #all
        all = nil; collectgarbage('collect')

        if cnt >= #core.project_files
        * iwdt / config.treeview_size then
